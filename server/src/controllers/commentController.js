...
const update = async (req, res) => {
  ...
  if (!current) throw new AppError(404, "COMMENT_NOT_FOUND", "Comentário não encontrado");
  if (current.userId !== req.user.id) throw new AppError(403, "FORBIDDEN", "Você não pode editar este comentário");
...
const remove = async (req, res) => {
  ...
  if (current.userId !== req.user.id && req.user.role !== "ADMIN") {
    throw new AppError(403, "FORBIDDEN", "Você não pode excluir este comentário");
  }
...
const like = async (req, res) => {
  ...
  if (!comment) throw new AppError(404, "COMMENT_NOT_FOUND", "Comentário não encontrado");
  const existing = await prisma.commentLike.findUnique({ where: { userId_commentId: { userId: req.user.id, commentId } } });
  if (existing) throw new AppError(409, "COMMENT_ALREADY_LIKED", "Comentário já curtido");
...
const unlike = async (req, res) => {
  ...
  if (!result.count) throw new AppError(404, "COMMENT_LIKE_NOT_FOUND", "Curtida não encontrada");
  const likeCount = await prisma.commentLike.count({ where: { commentId } });
