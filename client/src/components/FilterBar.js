...
export default function FilterBar({ filters, genres = [], onChange, showGenre = true }) {
  ...
  return <Bar><div className="bar-head"><div className="title"><span><SlidersHorizontal size={16} /></span> Refine catalog</div><button className="reset" type="button" onClick={reset}><RotateCcw size={14} /> Clear filters</button></div>{showGenre && <label>Genre<select value={filters.genre || ""} onChange={set("genre")}><option value="">All</option>{genres.map((genre) => <option key={genre.id} value={genre.id}>{genre.name}</option>)}</select></label>}<label>Year<input type="number" min="1900" max={new Date().getFullYear() + 5} placeholder="e.g. 2024" value={filters.year || ""} onChange={set("year")} /></label><label>Minimum rating<select value={filters.voteMin || ""} onChange={set("voteMin")}><option value="">Any rating</option>{[5,6,7,8,9].map((value) => <option key={value} value={value}>{value}+</option>)}</select></label><label>Language<select value={filters.language || ""} onChange={set("language")}><option value="">All</option><option value="pt">Portuguese</option><option value="en">English</option><option value="es">Spanish</option><option value="fr">French</option><option value="ja">Japanese</option><option value="ko">Korean</option></select></label><label className="sort">Sort by<select value={filters.sort || "popularity"} onChange={set("sort")}><option value="popularity">Popularity</option><option value="rating">Rating</option><option value="release">Release date</option></select></label></Bar>;
}
