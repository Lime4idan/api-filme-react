...
export default function HeroBanner({ movie, trailerKey }) {
  ...
  return <Hero $image={imageUrl(movie.backdrop_path, "original")}><div className="edition">MovieHub selection<strong>01</strong></div><div className="content"><span className="spotlight"><Sparkles size={14} /> Pick of the week</span><h1>{movie.title}</h1><div className="meta"><span className="score"><Star size={15} fill="currentColor" />{Number(movie.vote_average || 0).toFixed(1)} TMDB</span><span><Calendar size={15} />{movieYear(movie.release_date)}</span><span className="language">{movie.original_language?.toUpperCase()}</span></div><p>{movie.overview || "Discover the story, cast, and every detail behind this featured title."}</p><div className="actions"><Button as={Link} to={`/filme/${movie.id}`}><Play size={17} fill="currentColor" /> Explore movie</Button><FavoriteButton movie={movie} />{trailerKey && <Button as="a" href={`https://www.youtube.com/watch?v=${trailerKey}`} target="_blank" rel="noreferrer" $variant="ghost"><Info size={18} /> Watch trailer</Button>}</div></div><span className="scroll-cue">Scroll to discover <ChevronDown size={16} /></span></Hero>;
}
