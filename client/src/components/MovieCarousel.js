...
export default function MovieCarousel({ title, subtitle, movies = [], to, ranked = false }) {
  ...
  return <Section><div className="head"><div><div className="title-row"><span className="accent" /><h2>{title}</h2></div>{subtitle && <p className="subtitle">{subtitle}</p>}</div><div className="head-actions"><button className="rail-button" onClick={() => scroll(-1)} aria-label={`Voltar em ${title}`}><ChevronLeft size={18} /></button><button className="rail-button" onClick={() => scroll(1)} aria-label={`Avançar em ${title}`}><ChevronRight size={18} /></button>{to && <Link className="all" to={to}>Ver tudo <ArrowRight size={16} /></Link>}</div></div><div className="rail" ref={rail}>{movies.map((movie, index) => <MovieCard key={movie.id || movie.tmdbMovieId} movie={movie} rank={ranked ? index + 1 : undefined} />)}</div></Section>;
}
