...
export default function CommentCard({ comment, canModerate, onLike, onEdit, onDelete }) {
  ...
  const date = new Date(comment.createdAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" });
  return <Card><div className="avatar">{comment.author.avatarUrl ? <img src={comment.author.avatarUrl} alt="" /> : comment.author.name.charAt(0).toUpperCase()}</div><div><div className="head"><div><strong>{comment.author.name}</strong><br /><small>{date}{comment.updatedAt !== comment.createdAt ? " · edited" : ""}</small></div></div>{editing ? <><textarea value={content} maxLength={1000} aria-label="Edit comment" onChange={(event) => setContent(event.target.value)} /><div className="edit-actions"><Button $variant="ghost" onClick={() => { setEditing(false); setContent(comment.content); }}>Cancel</Button><Button onClick={() => { onEdit(comment.id, content); setEditing(false); }} disabled={!content.trim()}>Save</Button></div></> : <p>{comment.content}</p>}<div className="actions"><button className={`link ${comment.likedByMe ? "active" : ""}`} onClick={() => onLike(comment)} aria-pressed={comment.likedByMe}><Heart size={16} fill={comment.likedByMe ? "currentColor" : "none"} /> {comment.likeCount}</button>{comment.canEdit && !editing && <button className="link" onClick={() => setEditing(true)}><Pencil size={15} /> Edit</button>}{(comment.canEdit || canModerate) && <button className="link" onClick={() => onDelete(comment)}><Trash2 size={15} /> Delete</button>}</div></div></Card>;
}
