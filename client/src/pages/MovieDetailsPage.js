...
export default function MovieDetailsPage() {
  ...
  const { movie } = data;
  return <Wrap><Backdrop $image={imageUrl(movie.backdrop_path, "original")}><button className="back" onClick={() => navigate(-1)}><ArrowLeft size={16} /> Voltar</button><div className="hero"><div className="hero-poster"><SafeImage src={imageUrl(movie.poster_path)} alt={`Pôster de ${movie.title}`} fallback={<span className="poster-placeholder"><Film size={48} /></span>} /></div><div><div className="genres">{movie.genres?.map((genre) => <Chip key={genre.id}>{genre.name}</Chip>)}</div><h1>{movie.title}</h1>{movie.original_title !== movie.title && <p className="tagline">Título original: {movie.original_title}</p>}<div className="meta"><span className="tmdb"><Star size={15} fill="currentColor" /> {Number(movie.vote_average || 0).toFixed(1)} · {movie.vote_count?.toLocaleString("pt-BR")} votos</span><span><Calendar size={16} /> {formatDate(movie.release_date)}</span><span><Timer size={16} /> {formatRuntime(movie.runtime)}</span></div><p className="overview">{movie.overview || "Sinopse não disponível em português."}</p><div className="actions"><Button as="a" href={trailer ? "#trailer" : undefined} disabled={!trailer}><Play size={17} fill="currentColor" /> Assistir trailer</Button><FavoriteButton movie={movie} /><Button $variant="ghost" onClick={() => user ? setListModal(true) : navigate("/login", { state: { from: `/filme/${id}` } })}><ListPlus size={18} /> Adicionar à lista</Button></div></div></div></Backdrop><Content><div><h2>Por trás da história</h2><div className="facts"><div className="fact"><span><Film size={15} /> Status</span><strong>{movie.status || "Não informado"}</strong></div><div className="fact"><span><Languages size={15} /> Idioma original</span><strong>{movie.original_language?.toUpperCase() || "—"}</strong></div><div className="fact"><span><Globe2 size={15} /> Países</span><strong>{movie.production_countries?.map((item) => item.name).join(", ") || "Não informados"}</strong></div><div className="fact"><span><UsersRound size={15} /> Direção</span><strong>{director?.name || "Não informada"}</strong></div><div className="fact"><span><Building2 size={15} /> Produção</span><strong>{movie.production_companies?.slice(0, 2).map((item) => item.name).join(", ") || "Não informada"}</strong></div><div className="fact"><span><TrendingUp size={15} /> Popularidade</span><strong>{Math.round(movie.popularity || 0).toLocaleString("pt-BR")}</strong></div></div><div className="credits"><h2>Elenco principal</h2><div className="people">{cast.map((person) => <div className="person" key={person.credit_id}><div className="face"><SafeImage src={imageUrl(person.profile_path, "w185")} alt={person.name} fallback={<UsersRound />} loading="lazy" /></div><strong>{person.name}</strong><small>{person.character || "Elenco"}</small></div>)}</div></div></div><aside className="community"><RatingSelector movieId={id} /><div className="community-note"><strong><Star size={16} /> Sua opinião importa</strong>As avaliações da comunidade ajudam outras pessoas a encontrar a próxima grande história.</div></aside></Content>{trailer && <Trailer id="trailer"><h2>Assista ao trailer</h2><div className="frame"><iframe src={`https://www.youtube-nocookie.com/embed/${trailer.key}`} title={`Trailer de ${movie.title}`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div></Trailer>}<MovieCarousel title="Você também pode gostar" subtitle="Recomendados a partir deste filme" movies={data.recommendations.results} /><MovieCarousel title="No mesmo universo" subtitle="Histórias com uma atmosfera parecida" movies={data.similar.results.filter((item) => !data.recommendations.results.some((other) => other.id === item.id))} /><CommentSection movieId={id} />{listModal && <AddToListModal movie={movie} onClose={() => setListModal(false)} />}</Wrap>;
}
