...
export default function AdminPage() {
  ...
  if (!data) return <Page><p>Loading dashboard...</p></Page>;
  const statItems = [["users", "Users", UsersRound], ["comments", "Comments", MessageSquareText], ["lists", "Lists", ListVideo], ["favorites", "Favorites", Heart], ["ratings", "Ratings", Star]];
  const remove = async () => { try { await api.delete(`/admin/comments/${pending.id}`); setData((current) => ({ ...current, totals: { ...current.totals, comments: current.totals.comments - 1 }, recentComments: current.recentComments.filter((item) => item.id !== pending.id) })); toast.success("Comment removed"); } catch (err) { toast.error(err.message); } finally { setPending(null); } };
  const toggleUser = async (user) => { try { const { data: result } = await api.put(`/admin/users/${user.id}/status`, { isActive: !user.isActive }); setData((current) => ({ ...current, recentUsers: current.recentUsers.map((item) => item.id === user.id ? { ...item, isActive: result.user.isActive } : item) })); toast.success(result.user.isActive ? "User reactivated" : "User deactivated"); } catch (err) { toast.error(err.message); } };
  return <Page><PageHeader><div><Eyebrow>MovieHub operations</Eyebrow><h1>Admin dashboard</h1><p>Essential metrics and lightweight tools for a healthy community.</p></div><Shield size={36} /></PageHeader><Stats>{statItems.map(([key,label,Icon]) => <div className="stat" key={key}><Icon size={20} /><strong>{data.totals[key]}</strong><span>{label}</span></div>)}</Stats><Columns><Panel><h2><MessageSquareText size={20} /> Recent comments</h2>{data.recentComments.map((comment) => <div className="row" key={comment.id}><div><strong>{comment.user.name}</strong><p className="comment">{comment.content}</p><p>Movie #{comment.tmdbMovieId} · {formatDate(comment.createdAt)}</p></div><Button $variant="ghost" onClick={() => setPending(comment)} aria-label="Delete comment"><Trash2 size={16} /></Button></div>)}</Panel><Panel><h2><UserRound size={20} /> Recent users</h2>{data.recentUsers.map((user) => <div className="row" key={user.id}><div><strong>{user.name}</strong><p>{user.email} · {user.role}</p></div><button className={`status ${user.isActive ? "active" : ""}`} onClick={() => toggleUser(user)}>{user.isActive ? "Active" : "Deactivated"}</button></div>)}</Panel></Columns>{pending && <ConfirmModal title="Remove comment?" message="Moderation will permanently delete this comment and its likes." confirmLabel="Remove" onConfirm={remove} onClose={() => setPending(null)} />}</Page>;
}
