...
export default function AuthPage({ mode }) {
  ...
  return <Screen><section className="art"><Brand className="art-brand" /><div className="poster-stack" aria-hidden="true"><div className="poster" /><div className="poster-card"><span>Curadoria pessoal</span><strong>Seu cinema. Suas regras.</strong><small>Listas, notas e descobertas em um só lugar.</small></div></div><div className="brand-copy"><h1>Histórias que ficam <em>com você.</em></h1><p>Descubra universos, monte sua curadoria e transforme cada sessão em algo que vale compartilhar.</p><div className="benefits"><span><Check size={15} /> Listas ilimitadas</span><span><Check size={15} /> Recomendações pessoais</span><span><Check size={15} /> Comunidade cinéfila</span></div></div></section><section className="form-side"><div className="form-card"><span className="form-kicker">{registerMode ? <Sparkles size={14} /> : <Film size={14} />} {registerMode ? "Entre para o clube" : "Sua sessão continua"}</span><h1>{registerMode ? "Crie seu espaço" : "Bem-vindo de volta"}</h1><p>{registerMode ? "Uma curadoria de cinema tão única quanto o seu gosto." : "Entre para continuar descobrindo grandes histórias."}</p><FormGrid onSubmit={submit} noValidate>{error && <div className="error" id="form-error" role="alert">{error}</div>}{registerMode && <Field>Nome<input name="name" value={values.name} onChange={change} minLength={2} maxLength={80} required autoComplete="name" aria-describedby={error ? "form-error" : undefined} placeholder="Como devemos chamar você?" /></Field>}<Field>E-mail<input name="email" type="email" value={values.email} onChange={change} required autoComplete="email" placeholder="voce@exemplo.com" /></Field><Field>Senha<div className="password-field"><input name="password" type={showPassword ? "text" : "password"} value={values.password} onChange={change} minLength={registerMode ? 8 : 1} required autoComplete={registerMode ? "new-password" : "current-password"} placeholder="Sua senha" /><button className="reveal" type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div>{registerMode && <span>Use pelo menos 8 caracteres.</span>}</Field>{registerMode && <Field>Confirme a senha<input name="confirmPassword" type={showPassword ? "text" : "password"} value={values.confirmPassword} onChange={change} minLength={8} required autoComplete="new-password" placeholder="Repita sua senha" /></Field>}<Button className="submit" disabled={busy}>{busy ? "Preparando sua sessão..." : registerMode ? "Criar minha conta" : "Entrar"}</Button></FormGrid><span className="secure"><LockKeyhole size={13} /> Seus dados são protegidos com criptografia</span><p className="switch">{registerMode ? "Já tem uma conta? " : "Ainda não tem conta? "}<Link to={registerMode ? "/login" : "/cadastro"}>{registerMode ? "Entrar" : "Criar gratuitamente"}</Link></p></div></section></Screen>;
}
