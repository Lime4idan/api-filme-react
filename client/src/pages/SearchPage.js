...
export default function SearchPage() {
  ...
  return <Page><SearchHeader><div><Eyebrow>Advanced search</Eyebrow><h1>{filters.query ? `Results for “${filters.query}”` : "Find your next story"}</h1><p>Search the TMDB catalog and filter across multiple result pages.</p></div>{data?.totalResults > 0 && <span className="count">{data.totalResults.toLocaleString("en-US")} results</span>}</SearchHeader>{filters.query ? <FilterBar filters={filters} genres={genres} onChange={update} /> : null}{loading ? <LoadingSkeleton /> : error ? <ErrorState message={error} retry={() => update(filters)} /> : data?.results?.length ? <><MovieGrid movies={data.results} /><Pagination page={data.page} totalPages={data.totalPages} onChange={(page) => update({ ...filters, page })} /></> : <EmptyState title={filters.query ? "No results" : "Start with the search bar"} message={filters.query ? "Check the title or try less restrictive filters." : "Search for a title to see suggestions and complete results."} />}</Page>;
}
