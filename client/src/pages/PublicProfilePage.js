...
export default function PublicProfilePage() {
  ...
  if (!data) return <Page><p>Loading profile...</p></Page>;
  const { user, lists } = data;
  return <Page><PageHeader><div><Eyebrow>Community profile</Eyebrow><h1>{user.name}</h1><p>Public curation and participation on MovieHub.</p></div></PageHeader><Profile><Panel className="identity"><div className="avatar">{user.avatarUrl ? <img src={user.avatarUrl} alt={`Avatar for ${user.name}`} /> : <UserRound size={38} />}</div><h2>{user.name}</h2><p className="muted">{user.bio || "Passionate about great stories."}</p><p className="muted"><CalendarDays size={14} /> Since {formatDate(user.createdAt)}</p><div className="stats"><span><ListVideo size={17} />{user.stats?.lists || 0}</span><span><MessageSquareText size={17} />{user.stats?.comments || 0}</span><span><Star size={17} />{user.stats?.ratings || 0}</span></div></Panel><div><h2>Public lists</h2><div className="lists">{lists.map((list) => <Link className="list" key={list.id} to={`/lista/${list.shareCode}`}><div><h3>{list.name}</h3><p>{list.description || "Public list"}</p></div><strong>{list._count.items} movies</strong></Link>)}</div></div></Profile></Page>;
}
