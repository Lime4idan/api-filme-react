...
export default function ListDetailPage() {
  ...
  if (!list) return <Page><p>Carregando lista...</p></Page>;
  const save = async (event) => {
    event.preventDefault();
    try { const { data } = await api.put(`/lists/${id}`, { name: list.name, description: list.description || null, isPublic: list.isPublic }); setList((current) => ({ ...current, ...data.list })); toast.success("Lista atualizada"); } catch (err) { toast.error(err.message); }
  };
  ...
  const remove = async () => {
    try { await api.delete(`/lists/${id}/items/${pending.tmdbMovieId}`); setList((current) => ({ ...current, items: current.items.filter((item) => item.tmdbMovieId !== pending.tmdbMovieId) })); toast.success("Filme removido da lista"); } catch (err) { toast.error(err.message); } finally { setPending(null); }
  };
  const pageSize = 20;
  ...
  return <Page><PageHeader><div><Eyebrow>{list.isPublic ? "Lista compartilhável" : "Coleção privada"}</Eyebrow><h1>{list.name}</h1><p>{list.items.length} {list.items.length === 1 ? "filme" : "filmes"} em uma ordem só sua.</p></div>{list.isPublic && <ShareButton title={list.name} text={list.description} url={`${window.location.origin}/lista/${list.shareCode}`} />}</PageHeader><Editor as="form" onSubmit={save}><h2>Detalhes da lista</h2><div className="fields"><Field>Nome<input value={list.name} maxLength={80} onChange={(event) => setList({ ...list, name: event.target.value })} required /></Field><Field>Descrição<input value={list.description || ""} maxLength={500} onChange={(event) => setList({ ...list, description: event.target.value })} /></Field><label className="public"><input type="checkbox" checked={list.isPublic} onChange={(event) => setList({ ...list, isPublic: event.target.checked })} /> {list.isPublic ? <Eye size={17} /> : <EyeOff size={17} />} Pública</label></div><div className="save"><Button><Save size={17} /> Salvar alterações</Button></div></Editor>{list.items.length ? <><List>{visibleItems.map((item, localIndex) => { const index = (page - 1) * pageSize + localIndex; return <div className="item" key={item.id}><span className="position">{index + 1}</span><SafeImage className="poster" src={imageUrl(item.posterPath, "w154")} alt={`Pôster de ${item.title}`} /><div className="info"><h3><button style={{ all: "unset", cursor: "pointer" }} onClick={() => navigate(`/filme/${item.tmdbMovieId}`)}>{item.title}</button></h3><span>{movieYear(item.releaseDate)} · ★ {Number(item.voteAverage || 0).toFixed(1)}</span></div><div className="actions"><button onClick={() => move(index, -1)} disabled={index === 0} aria-label={`Mover ${item.title} para cima`}><ArrowUp size={16} /></button><button onClick={() => move(index, 1)} disabled={index === list.items.length - 1} aria-label={`Mover ${item.title} para baixo`}><ArrowDown size={16} /></button><button onClick={() => setPending(item)} aria-label={`Remover ${item.title}`}><Trash2 size={16} /></button></div></div>; })}</List><Pagination page={page} totalPages={totalPages} onChange={setPage} /></> : <EmptyState title="Lista vazia" message="Abra um filme e use “Adicionar à lista” para começar esta coleção." />}{pending && <ConfirmModal title="Remover filme?" message={`“${pending.title}” será retirado desta lista.`} confirmLabel="Remover" onConfirm={remove} onClose={() => setPending(null)} />}</Page>;
}
